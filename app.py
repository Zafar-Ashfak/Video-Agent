...
:root {{
    ...
    --surface-2: {theme_vars["surface-2"]};
...
.stApp::before {{
    ...
    background-image:
        linear-gradient({theme_vars["grid"]} 1px, transparent 1px),
        linear-gradient(90deg, {theme_vars["grid"]} 1px, transparent 1px);
...
.hero-title {{
    ...
    line-height: 1.1;
    margin: 0;
    background: linear-gradient(135deg, {theme_vars["hero-start"]} 0%, var(--accent-glow) 50%, var(--accent-2) 100%);
...
/* Center the main hero subtitle under the title */
.main-hero {{
    text-align: center;
}}
...
/* Keep the theme tooltip readable in both themes */
...
/* URL/file-path input: black text in light mode, normal theme text in dark mode. */
[data-testid="stSidebar"] .stTextInput > div > div > input {{
    color: {theme_vars["input-text"]} !important;
    -webkit-text-fill-color: {theme_vars["input-text"]} !important;
}}
...
with theme_col:
    ...
    mode = "Switch to light mode" if st.session_state.theme == "dark" else "Switch to dark mode"
    st.markdown('<div class="theme-toggle-wrap">', unsafe_allow_html=True)
    if st.button(icon, key="theme_toggle", help=mode):
